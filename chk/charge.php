...
$anonmask = array(
    "currency"=>"usd",
    "desc"=>"anonmask donation",
    "currency_symbol"=>$cur,
    "amount"=>$amount ==="min" ? 50 : $amount * 100, 
    "country"=>"India", "sk"=>$sk);
...
$status = '<font color=blue>Approve CVV<br></font>';
$resmsg = $cur.$amount.' <font color=blue>Charged!</font> ';
echo ' <p class="uk-margin-small-top">'.$status.' : '.$resmsg.' : ' . $lista . ' : '.$country.' :<br>'.$risklevel.' <br><a class="receipt" href="'.$receipturl.'">Get Receipt</a> - </p>';
exit;
}elseif ((strpos($r2,'insufficient_funds')) || (strpos($r3,'insufficient_funds'))){
$status = '<font color=blue>Approve CVV</font><br>';
$resmsg = '<font color=lime>INSUFFICIENT FUND</font><br>';



}elseif (strpos($r3, "incorrect_cvc") || strpos($r2, "incorrect_cvc")) {
$status = '<font color=yellow>Approve CCN<br></font>';
$resmsg = '<font color=blue>INCORRECT CVC</font><br>';
}
elseif (strpos($r1, 'generic_decline')){
$status = '<font color=blue>OTP</font><br>';
$resmsg = '<font color=blue>CHARGED CANCELED</font><br><font color=orange>TRIAL SITE ONLY</font><br>';

}

elseif (strpos($r1, 'test_mode_live_card')){
$status = '<font color=red>RESULT</font><br>';
$resmsg = '<font color=red>TEST_MODE LIVE_CARD</font>';
}

elseif (strpos($r1, 'testmode_charges_only')){
$status = '<font color=red>STATUS</font><br>';
$resmsg = '<font color=red>TEST_MODE CHARGE ONLY</font>';
}

elseif(strpos($r1, "invalid_request_error" )) {
$status = '<font color=red>DEAD</font><br>';
$resmsg = '<font color=red>™INVALID REQUEST ERROR™<br>CARD IS INCORRECT</font>';
}
...
$resmsg = '<font color=red>YOUR SK is DEAD</font>';
}

elseif(strpos($r1, "api_key_expired" )) {
$status = '<font color=red>STATUS</font>';
$resmsg = '<font color=red>YOUR API KEY EXPIRED</font>';
}

else {
$status = '<font color=red>DECLINED<br></font>';
$resmsg = '<font color=red></font>';

}
    echo '<p class="uk-margin-small-top"><font color=yellow>'.$status.''.$lista .'<br>'. $bindata1.''.$resmsg.'<font color=red>'.$d_code3.'</font><br>'.$risklevel.'';
